...
public enum JavaDataTypeGenerateValueEnum {
  ...
  /** 运行的方法 */
  private String runFuncation;

  JavaDataTypeGenerateValueEnum(StandardTypeEnum key, String runFuncation) {
    this.key = key;
    this.runFuncation = runFuncation;
  }
  ...
  public static String getGenerateFun(StandardTypeEnum key, int length) {
    ...
    int outNewValue = RandomUtils.nextInt(1, outLength);

    outLenght = String.valueOf(outNewValue);

    for (JavaDataTypeGenerateValueEnum item : values()) {
      if (item.getKey().equals(key)) {
        String itemValue = item.getRunFuncation();
        if (itemValue.indexOf("#length#") != -1) {
          return itemValue.replaceAll("#length#", outLenght);
        } else {
          return itemValue;
        }
      }
    ...
    return "RandomStringUtils.randomNumeric(" + outLenght + ")";
  }
  ...
  public String toString() {
    ...
    sb.append(", runFuncation='").append(runFuncation).append('\'');
    sb.append('}');
    return sb.toString();
  }
}
